...
def postprocess_args(args):
    ...
    save_path1 = '-'.join([args.dataset, args.backbone, args.classifier,
                           '{:02d}w{:02d}s{:02}q-{}-{}'.format(args.way, args.shot, args.query, args.kd_loss,
                                                               args.kd_weight)])
...
def get_command_line_parser():
    ...
    parser.add_argument('--episodes_per_epoch', type=int, default=2000)
    parser.add_argument('--num_eval_episodes', type=int, default=600)
    parser.add_argument('--classifier', type=str, default='RelationNet',
                        choices=['DN4', 'ProtoNet', 'RelationNet'])  # , 'MatchNet', 'ProtoNet', 'BILSTM', 'DeepSet', 'GCN', 'FEAT', 'FEATSTAR', 'SemiFEAT', 'SemiProtoFEAT']) # None for MatchNet or ProtoNet
    parser.add_argument('--use_euclidean', action='store_true', default=False)
    parser.add_argument('--backbone', type=str, default='Vit',
                        choices=['ConvNet', 'Res12', 'Vit'])
    ...
    parser.add_argument('--init_weights', type=str, default='./Res12-pre.pth')
    ...
    parser.add_argument("--teacher_backbone_class", type=str, default='Res12', choices=['Res12'])
    parser.add_argument('--teacher_init_weights', type=str, default=None)
    parser.add_argument('--kd_loss', type=str, default='KD',
                        choices=['KD', 'ACD', 'LFD', 'IRD'])
    parser.add_argument('--kd_weight', type=float, default=1.0)

    # argument for Slim-Resnet12
    parser.add_argument('--is_prune', action="store_true", default=False)
    parser.add_argument('--remove_ratio', type=float, default=0.5)
